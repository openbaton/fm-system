...
public class MonitoringManagerImpl implements MonitoringManager {
  ...
  private boolean isVNFCMonitored(String hostname) {
    for (ManagedNetworkServiceRecord mnsr : mnsrRepo.findAll()) {
      ...
      for (Map.Entry<String, ThresholdHostnames> entry : mnsr.getHostnames().entrySet()) {
        if (entry.getValue().getHostnames().contains(hostname)) return true;
      }
    }
    return false;
  }
  ...
  public boolean isVNFThreshold(String thresholdId) {
    for (ManagedNetworkServiceRecord mnsr : mnsrRepo.findAll()) {
      if (mnsr.getVnfTriggerId().contains(thresholdId)) return true;
    }
    return false;
  }
  ...
  private class MonitoringThreadCreator implements Runnable {
      ...
      try {
        ...
        for (VirtualNetworkFunctionRecord vnfr : nsr.getVnfr()) {
          ...
          for (VirtualDeploymentUnit vdu : vnfr.getVdu()) {
            ...
            if (vdu.getMonitoring_parameter() == null) continue;
            if (vdu.getMonitoring_parameter().isEmpty()) continue;
            ...
            for (VNFCInstance vnfcInstance : vdu.getVnfc_instance()) {
              ...
              if (vnfcInstance.getState() != null
                  && vnfcInstance.getState().equalsIgnoreCase("standby")) {
                continue;
            ...
            if (objectSelection.getObjectInstanceIds().isEmpty()) continue;
            log.debug(
                "the vnfc instances to be monitored are: "
                    + objectSelection.getObjectInstanceIds());

            Set<String> monitoringParamentersWithoutPeriod =
                getMonitoringParamentersWithoutPeriod(vdu.getMonitoring_parameter(), vdu);
              ...
              for (FaultManagementPolicy vnffmp : vdu.getFault_management_policy()) {
                ...
                for (Criteria criteria : vnffmp.getCriteria()) {
                  ...
                  ThresholdDetails thresholdDetails =
                      new ThresholdDetails(
                          function,
                          criteria.getComparison_operator(),
                          vnffmp.getSeverity(),
                  ...
                  if (criteria.getVnfc_selector() == VNFCSelector.at_least_one)
                    for (String host : objectSelection.getObjectInstanceIds()) {
                      ...
                      thresholdId =
                          monitoringPluginCaller.createThreshold(
                              objs,
                              performanceMetric,
    ...
    private Set<String> getMonitoringParamentersWithoutPeriod(
        Set<String> monitoring_parameter, VirtualDeploymentUnit vdu) {
