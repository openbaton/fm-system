...
public class EventSubscriptionManagerImpl implements EventSubscriptionManger,CommandLineRunner,ApplicationListener<ContextClosedEvent> {
    ...
    public String subscribe(NetworkServiceRecord networkServiceRecord, Action action) throws SDKException {
        EventEndpoint eventEndpoint = createEventEndpoint("FM-nsr-"+action,EndpointType.RABBIT,action, RabbitEventReceiverConfiguration.queueName_vnfEvents);
    ...
    public String subscribe(VirtualNetworkFunctionRecord virtualNetworkFunctionRecord, Action action) throws SDKException {
        EventEndpoint eventEndpoint = createEventEndpoint("FM-vnfr-"+action,EndpointType.RABBIT,action, RabbitEventReceiverConfiguration.queueName_vnfEvents);
    ...
    public void run(String... args) throws Exception {
        EventEndpoint eventEndpointInstantiateFinish = createEventEndpoint("FM-nsr-INSTANTIATE_FINISH", EndpointType.RABBIT, Action.INSTANTIATE_FINISH, RabbitEventReceiverConfiguration.queueName_eventInstatiateFinish);
        //TODO Subscribe for REALEASE_RESOURCES_FINISH only for nsr which require fault management
        EventEndpoint eventEndpointReleaseResourcesFinish = createEventEndpoint("FM-nsr-RELEASE_RESOURCES_FINISH",EndpointType.RABBIT,Action.RELEASE_RESOURCES_FINISH, RabbitEventReceiverConfiguration.queueName_eventResourcesReleaseFinish);
        unsubscriptionIdINSTANTIATE_FINISH = sendSubscription(eventEndpointInstantiateFinish);
        unsubscriptionIdRELEASE_RESOURCES_FINISH = sendSubscription(eventEndpointReleaseResourcesFinish);
